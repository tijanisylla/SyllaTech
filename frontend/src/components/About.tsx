              ...
              <div className={`absolute bottom-6 ${isRTL ? 'right-6' : 'left-6'}`}>
                <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={isRTL ? 'text-right' : ''}>
                    <p className="text-white font-bold text-lg">{t('about.established')} {companyInfo.established}</p>
                    <p className="text-white/70 text-sm">{t('about.serving')}</p>
                  </div>
                </div>
              </div>
            </div>
            {/* Floating Card */}
            <div className={`absolute -bottom-8 ${isRTL ? '-left-8' : '-right-8'} bg-white rounded-2xl p-6 shadow-xl max-w-xs hidden lg:block`}>
              <div className={`flex items-center gap-4 mb-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <div className="w-12 h-12 rounded-full bg-teal-100 flex items-center justify-center">
                  <Users className="w-6 h-6 text-teal-600" />
                </div>
                <div className={isRTL ? 'text-right' : ''}>
                  <p className="font-bold text-slate-900">{t('about.expertTeam')}</p>
                  <p className="text-sm text-slate-500">25+ {t('about.professionals')}</p>
                </div>
              </div>
              <p className={`text-slate-600 text-sm ${isRTL ? 'text-right' : ''}`}>
                {t('about.diverseTalents')}
              </p>
            </div>
          </div>
          ...
          <div className={`space-y-8 ${isRTL ? 'lg:col-start-1' : ''}`}>
            ...
            <div className="group p-8 rounded-2xl bg-white shadow-lg hover:shadow-xl transition-shadow duration-300 border border-slate-100">
              <div className={`flex items-start gap-5 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={isRTL ? 'text-right' : ''}>
                  <h3 className="text-2xl font-bold text-slate-900 mb-3">{t('about.mission')}</h3>
            ...
            <div className="group p-8 rounded-2xl bg-white shadow-lg hover:shadow-xl transition-shadow duration-300 border border-slate-100">
              <div className={`flex items-start gap-5 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={isRTL ? 'text-right' : ''}>
                  <h3 className="text-2xl font-bold text-slate-900 mb-3">{t('about.vision')}</h3>
                  <p className="text-slate-600 leading-relaxed">
                    {t('about.visionText')}
                  </p>
                </div>
            ...
            {/* Location Badge */}
            <div className={`inline-flex items-center gap-3 px-6 py-3 bg-slate-900 rounded-full ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className="w-3 h-3 rounded-full bg-amber-400 animate-pulse" />
              <span className="text-white font-medium">{t('about.basedIn')} {companyInfo.location}</span>
            </div>
          </div>
        </div>
      </div>
