...
const Admin: React.FC = () => {
  ...
  const fetchData = async (apiKey: string, isPolling = false) => {
    ...
    try {
      ...
      if (!isInitialLoadRef.current) {
        ...
        const addedTotal = addedNews + addedBook + addedContact;
        if (addedTotal > 0) {
          setUnreadNotificationCount((prev) => prev + addedTotal);
          const items: { ts: string }[] = [];
          newNews.slice(-addedNews).forEach((n: NewsletterItem) => items.push({ ts: n.timestamp }));
          newBook.slice(-addedBook).forEach((b: BookingItem) => items.push({ ts: b.timestamp }));
          newContact.slice(-addedContact).forEach((c: ContactItem) => items.push({ ts: c.timestamp }));
          const latest = items.sort((a, b) => new Date(b.ts).getTime() - new Date(a.ts).getTime())[0];
          if (latest) setLastNotificationTime(latest.ts);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#030712] via-[#0a0f1a] to-[#030712] overflow-x-hidden">
      {/* Header */}
      <header className="sticky top-0 z-10 border-b border-white/[0.06] bg-[rgba(8,12,24,0.92)] backdrop-blur-xl">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0">
            <Link to="/" className="flex items-center gap-1.5 sm:gap-2 text-slate-400 hover:text-cyan-400 transition-colors text-sm shrink-0" title="Back to site">
              <ArrowLeft className="w-4 h-4 shrink-0" />
              <span className="hidden sm:inline">Back to site</span>
            </Link>
            <img src="/logo.svg" alt="SyllaTech" className="h-6 sm:h-7 shrink-0" />
            <span className="inline-block px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-xs font-medium bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 shrink-0">
              Admin
            </span>
          </div>
          <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            ...
            <div className="relative" ref={notificationsPanelRef}>
              ...
              >
                ...
                {unreadNotificationCount > 0 && !showNotificationsPanel && (
                  <span
                    className="absolute -top-0.5 -right-0.5 flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-cyan-500 text-white text-[10px] font-bold"
                    title={lastNotificationTime ? formatRelativeTime(lastNotificationTime) : 'New notifications'}
                  >
                    {unreadNotificationCount > 99 ? '99+' : unreadNotificationCount}
                    {lastNotificationTime && (
                      <span className="font-normal opacity-90 text-[9px]">
                        · {formatRelativeTime(lastNotificationTime).replace(' ago', '')}
                      </span>
                    )}
                  </span>
                )}
              ...
              <AnimatePresence>
                {showNotificationsPanel && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 w-[min(20rem,calc(100vw-2rem))] max-h-[min(24rem,70vh)] overflow-hidden rounded-xl border border-white/[0.06] bg-[#0a0f1a] shadow-xl z-50 flex flex-col"
                  >
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl text-slate-400 hover:text-white hover:bg-white/[0.05] transition-colors text-sm"
            >
        ...
        {/* Section + Tabs */}
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 border-t border-white/[0.06]">
          <div className="flex gap-1 pt-2 overflow-x-auto scrollbar-thin pb-1 -mb-1" style={{ scrollbarWidth: 'thin' }}>
            <button
              onClick={() => setAdminSection('analytics')}
              className={cn(
                "flex items-center gap-2 px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm font-medium rounded-t-lg transition-colors whitespace-nowrap shrink-0",
          ...
          {adminSection === 'submissions' && (
            <div className="flex gap-1 overflow-x-auto pb-1 -mb-1" style={{ scrollbarWidth: 'thin' }}>
              {tabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={cn(
                    "flex items-center gap-2 px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-medium transition-colors border-b-2 -mb-px whitespace-nowrap shrink-0",
                    activeTab === tab.id
      ...
      <main className="max-w-[1280px] mx-auto px-4 sm:px-6 py-4 sm:py-8">
        ...
        {adminSection === 'analytics' && (
          ...
          >
            ...
            <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] hover:border-cyan-500/30 transition-colors p-4 sm:p-6">
        ...
        {adminSection === 'email' && (
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 sm:p-6 space-y-4 sm:space-y-6">
            <h2 className="text-lg font-semibold text-white flex items-center gap-2">
            ...
            <div className="flex justify-end">
              <button
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:opacity-90 text-white font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
              >
                {sendingEmail ? (
                  <>
        ...
        {adminSection === 'submissions' && activeTab === 'newsletter' && (
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-4 py-2 border-b border-slate-800 bg-slate-800/30">
              ...
              <table className="w-full min-w-[500px]">
                <thead>
                  <tr className="border-b border-slate-800">
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base">Email</th>
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base">Status</th>
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base hidden md:table-cell">Date</th>
                    <th className="text-right px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium w-20 sm:w-24 text-xs sm:text-base">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {newsletter.length === 0 ? (
                    ...
                    newsletter.map((item) => (
                      <tr key={item.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-white text-sm sm:text-base truncate max-w-[120px] sm:max-w-none">{item.email}</td>
                        ...
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-slate-500 text-sm hidden md:table-cell">{formatDate(item.timestamp)}</td>
            ...
            <div className="overflow-x-auto -mx-4 sm:mx-0">
              <table className="w-full min-w-[320px]">
                <thead>
                  <tr className="border-b border-slate-800">
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base">Email</th>
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base hidden sm:table-cell">Unsubscribed</th>
                    <th className="text-right px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium w-20 sm:w-24 text-xs sm:text-base">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {unsubscribed.length === 0 ? (
                    ...
                    unsubscribed.map((item) => (
                      <tr key={item.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-white text-sm sm:text-base truncate max-w-[140px] sm:max-w-none">{item.email}</td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-slate-500 text-sm hidden sm:table-cell">{formatDate(item.timestamp)}</td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-right">
                          <button
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
            ...
            <div className="overflow-x-auto -mx-4 sm:mx-0">
              <table className="w-full min-w-[520px]">
                <thead>
                  <tr className="border-b border-slate-800">
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base">Name</th>
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base">Email</th>
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base hidden lg:table-cell">Date & Time</th>
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base hidden md:table-cell">Business</th>
                    <th className="text-right px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium w-20 sm:w-24 text-xs sm:text-base">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {bookings.length === 0 ? (
                    ...
                    bookings.map((item) => (
                      <tr key={item.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-white text-sm sm:text-base truncate max-w-[100px] sm:max-w-none">{item.name}</td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-slate-300 text-sm sm:text-base truncate max-w-[100px] sm:max-w-none">{item.email}</td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-slate-400 text-sm hidden lg:table-cell">
                          {item.date} {item.time}
                        </td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-slate-500 text-sm hidden md:table-cell">{item.business || '—'}</td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button
              ...
              <table className="w-full min-w-[560px]">
                <thead>
                  <tr className="border-b border-slate-800">
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base">Name</th>
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base">Email</th>
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base hidden lg:table-cell">Business</th>
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base hidden md:table-cell">Message</th>
                    <th className="text-left px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium text-xs sm:text-base hidden sm:table-cell">Date</th>
                    <th className="text-right px-3 sm:px-4 py-2 sm:py-3 text-slate-400 font-medium w-20 sm:w-24 text-xs sm:text-base">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {contact.length === 0 ? (
                    ...
                    contact.map((item) => (
                      <tr key={item.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-white text-sm sm:text-base truncate max-w-[80px] sm:max-w-none">{item.name}</td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-slate-300 text-sm sm:text-base truncate max-w-[100px] sm:max-w-none">{item.email}</td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-slate-500 text-sm hidden lg:table-cell">{item.business || '—'}</td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-slate-400 text-sm max-w-[80px] sm:max-w-xs truncate hidden md:table-cell">{item.message}</td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-slate-500 text-sm hidden sm:table-cell">{formatDate(item.timestamp)}</td>
                        <td className="px-3 sm:px-4 py-2 sm:py-3 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button
      ...
      {/* Reply modal */}
      {replyTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 overflow-y-auto" onClick={() => setReplyTarget(null)}>
          <div className="w-full max-w-lg rounded-xl border border-white/[0.06] bg-[#0a0f1a] p-4 sm:p-6 shadow-xl my-4 sm:my-8 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
      ...
      {/* Delete confirmation */}
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80" onClick={() => setDeleteTarget(null)}>
          <div className="w-full max-w-md rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 sm:p-6 shadow-xl mx-2" onClick={(e) => e.stopPropagation()}>
      ...
      {/* Edit modal */}
      {editTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 overflow-y-auto" onClick={() => setEditTarget(null)}>
          <div className="w-full max-w-lg rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 sm:p-6 shadow-xl my-4 sm:my-8 max-h-[90vh] overflow-y-auto mx-2" onClick={(e) => e.stopPropagation()}>
